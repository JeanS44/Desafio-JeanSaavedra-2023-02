...
@section('title', 'Canciones Escuchadas')
@section('miga', 'Vista Principal del Panel de Canciones escuchadas por los Usuarios')
    ...
    <div class=" align-items-center justify-content-between mb-4">
        <table id="usersSongsTable" class="display nowrap table table-striped responsive" style="width:100%">
            <thead>
                <tr>
                    <th>Oyente</th>
                    <th>Canción Escuchada</th>
                    <th>¿Hace cuanto la escuchó?</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($users as $user)
                    @foreach ($user->canciones as $cancion)
                        <tr>
                            <td>{{ $user->name }}</td>
                            <td>{{ $cancion->title }}</td>
